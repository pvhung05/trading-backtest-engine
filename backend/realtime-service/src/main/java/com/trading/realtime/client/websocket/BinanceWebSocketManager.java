...
public class BinanceWebSocketManager {
	...
	private void processKlineMessage(String payload) {
		try {
			...
			Kline kline = Kline.builder()
					...
					.isClosed(Boolean.TRUE.equals(k.getIsClosed()))
					.build();
	...
	/**
	 * Periodically sends a ping message to all active WebSocket sessions
	 * to prevent timeout and maintain connectivity with Binance.
	 */
	@Scheduled(fixedRateString = "${binance.ws.heartbeat-interval-ms:30000}")
	public void sendHeartbeat() {
		if (!isTickerStreamActive() && getActiveKlineStreamCount() == 0) {
			return;
		}

		try {
			String pingPayload = objectMapper.writeValueAsString(BinanceWebSocketMessage.ping());
			TextMessage message = new TextMessage(pingPayload);

			tickerSession.values().stream()
					.filter(WebSocketSession::isOpen)
					.forEach(session -> {
						try {
							session.sendMessage(message);
							log.trace("Heartbeat sent to ticker session: {}", session.getId());
						} catch (IOException e) {
							log.debug("Failed to send heartbeat to ticker session {}: {}", session.getId(), e.getMessage());
						}
					});

			klineSessions.values().stream()
					.filter(WebSocketSession::isOpen)
					.forEach(session -> {
						try {
							session.sendMessage(message);
							log.trace("Heartbeat sent to kline session: {}", session.getId());
						} catch (IOException e) {
							log.debug("Failed to send heartbeat to kline session {}: {}", session.getId(), e.getMessage());
						}
					});
		} catch (Exception e) {
			log.error("Failed to construct or send heartbeat: {}", e.getMessage());
		}
	}
}
