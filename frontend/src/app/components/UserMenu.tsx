  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        ...
        className="w-8 h-8 bg-gradient-to-tr from-blue-600 to-indigo-500 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-xs hover:ring-2 hover:ring-blue-400/50 transition-all duration-150 active:scale-95 cursor-pointer"
        title={userName}
      >
      ...
      {open && (
        <div
          // absolute + mt-1.5 anchors the panel just under the avatar.
          // min-w-[210px] keeps it from collapsing when the user
          // name is short. shadow-xl + border mimic modern popovers.
          className="absolute left-0 mt-1.5 min-w-[210px] bg-white/95 dark:bg-gray-800/95 backdrop-blur-md rounded-xl border border-gray-200/80 dark:border-gray-700/80 shadow-xl py-1.5 z-50 overflow-hidden"
          role="menu"
        >
          ...
          <div className="p-1">
            <button
              type="button"
              onClick={toggleTheme}
              className="w-full flex items-center justify-between gap-2 px-3 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700/70 rounded-lg transition-all duration-150 text-left cursor-pointer"
              role="menuitemcheckbox"
              aria-checked={isDark}
            >
              <span className="flex items-center gap-2 font-medium">
                {isDark ? <Moon className="size-4 text-blue-400" /> : <Sun className="size-4 text-amber-500" />}
                Dark theme
              </span>
            ...
            {onLogout && (
              <>
                <div className="h-px bg-gray-100 dark:bg-gray-700/80 my-1 mx-1" />
                <button
                  type="button"
                  onClick={() => {
                    setOpen(false);
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 text-sm font-medium text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-lg transition-all duration-150 text-left cursor-pointer"
                >
                  <svg
                    className="size-4"
                    viewBox="0 0 24 24"
